...
PoduleConfigDialog::PoduleConfigDialog(wxWindow *parent, const podule_header_t *podule, const podule_config_t *config, bool running, int slot_nr, const char *prefix)
    : wxDialog(parent, -1, "Configure podule"), podule(podule), running(running), slot_nr(slot_nr), prefix(prefix) {
...
int podule_config_file_selector(void *window_p, const char *title, const char *default_path, const char *default_fn, const char *default_ext, const char *wildcard, char *dest, int dest_len,
                                int flags) {
    PoduleConfigDialog *dlg = static_cast<PoduleConfigDialog *>(window_p);
    int wx_flags = 0;
    ...
    wxString fn = wxFileSelector(title, default_path, default_fn, default_ext, wildcard, wx_flags, dlg);
