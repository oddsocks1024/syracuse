...
static const char *page_sizes[4] = {"4k", "8k", "16k", "32k"};

static const char *rom_speeds[4] = {"450ns", "325ns", "200ns", "200ns w/60ns nibble mode"};

static const char *refresh_modes[4] = {"Disabled", "Vblank only", "Disabled", "Continuous"};
...
void memc_debug_print(char *s) {
    sprintf(s,
            ...
            memctrl, page_sizes[pagesize], rom_speeds[(memctrl >> 4) & 3], rom_speeds[(memctrl >> 6) & 3], refresh_modes[(memctrl >> 8) & 3], (memctrl & (1 << 10)) ? "Enabled" : "Disabled",
            (memctrl & (1 << 11)) ? "Enabled" : "Disabled", (memctrl & (1 << 12)) ? "Enabled" : "Disabled", vinit << 2, vstart << 2, vend << 2, cinit << 2, sstart << 2, sendN << 2,
            vidc_get_current_vaddr() << 2, vidc_get_current_caddr() << 2, spos, ssend);
}
...
void memc_debug_print_cam(void) {
    ...
    for (int j = 0; j < nr_memcs; j++) {
        ...
        for (int i = 0; i < 128 / 4; i++) {
            ...
            sprintf(s, " [%02x] addr=%07x ppl=%i   [%02x] addr=%07x ppl=%i   [%02x] addr=%07x ppl=%i   [%02x] addr=%07x ppl=%i\n", i, memc_cam[offset].logical_addr, memc_cam[offset].ppl, i + 32,
                    memc_cam[offset + 32].logical_addr, memc_cam[offset + 32].ppl, i + 64, memc_cam[offset + 64].logical_addr, memc_cam[offset + 64].ppl, i + 96, memc_cam[offset + 96].logical_addr,
                    memc_cam[offset + 96].ppl);
