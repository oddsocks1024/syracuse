...
wxBEGIN_EVENT_TABLE(Frame, wxFrame) wxEND_EVENT_TABLE() wxIMPLEMENT_APP_NO_MAIN(App);
...
bool App::OnInit() {
    ...
    if (rom_establish_availability()) {
        wxMessageBox("No ROMs available\nSyracuse needs at least one ROM set present to run", "Syracuse", wxOK | wxCENTRE | wxSTAY_ON_TOP);
        exit(-1);
    }
...
Frame::Frame(App *app, const wxString &title, const wxPoint &pos, const wxSize &size) : wxFrame(NULL, wxID_ANY, title, pos, size, 0) {
    main_frame = this;
...
void Frame::OnMenuCommand(wxCommandEvent &event) {
    if (event.GetId() == XRCID("IDM_FILE_EXIT")) {
        ...
        if (firstfull) {
            ...
            arc_pause_main_thread();
            wxMessageBox("Use CTRL + END to return to windowed mode", "Syracuse", wxOK | wxCENTRE | wxSTAY_ON_TOP);
            arc_resume_main_thread();
        }
