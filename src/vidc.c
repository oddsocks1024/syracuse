...
enum { VIDC_HSYNC = 0, VIDC_FRONT_PORCH, VIDC_DISPLAY, VIDC_BACK_PORCH };
            ...
            if (vidc.displayon) {
                ...
                if (((vidc.cys >> 14) + 2) <= vidc.line && ((vidc.cye >> 14) + 2) > vidc.line) {
                    if (monitor_type == MONITOR_MONO) {
                        ...
                        for (xx = 0; xx < 64; xx += 4) {
                            if (temp & 3)
                                ((uint32_t *)bp)[x + xx] = ((uint32_t *)bp)[x + xx + 1] = ((uint32_t *)bp)[x + xx + 2] = ((uint32_t *)bp)[x + xx + 3] = hirescurcol[temp & 3];
                            temp >>= 2;
                        }
                        temp = ram[vidc.caddr++];
                        ...
                        switch (vidcr[VIDC_CR] & 0xF) {
                            ...
                            case 13:
                                ...
                                for (xx = 0; xx < 32; xx += 2) {
                                    if (temp & 3)
                                        ((uint32_t *)bp)[x + xx] = ((uint32_t *)bp)[x + xx + 1] = vidc.pal[(temp & 3) | 0x10];
                                    temp >>= 2;
                                }
                                temp = ram[vidc.caddr++];
                            ...
                            case 15:          /*Mode 28*/
                                ...
                                for (xx = 0; xx < 16; xx++) {
                                    if (temp & 3)
                                        ((uint32_t *)bp)[x + xx] = vidc.pal[(temp & 3) | 0x10];
                                    temp >>= 2;
                                }
                                temp = ram[vidc.caddr++];
                                for (xx = 16; xx < 32; xx++) {
