...
enum { CPU_ARM2 = 0, CPU_ARM250, CPU_ARM3_20, CPU_ARM3_25, CPU_ARM3_26, CPU_ARM3_30, CPU_ARM3_33, CPU_ARM3_35, CPU_ARM3_24, CPU_ARM3_36, CPU_ARM3_40, CPU_MAX };

const char *cpu_names[] = {"ARM2", "ARM250", "ARM3 @ 20 MHz", "ARM3 @ 25 MHz", "ARM3 @ 26 MHz", "ARM3 @ 30 MHz", "ARM3 @ 33 MHz", "ARM3 @ 35 MHz", "ARM3 @ 24 MHz", "ARM3 @ 36 MHz", "ARM3 @ 40 MHz"};
...
enum { MEMC_MEMC1 = 0, MEMC_MEMC1A_8, MEMC_MEMC1A_12, MEMC_MEMC1A_16, MEMC_MEMC1A_20, MEMC_MEMC1A_24, MEMC_MAX };

const char *memc_names[] = {"MEMC1", "MEMC1a (8 MHz)", "MEMC1a (12 MHz)", "MEMC1a (16 MHz - overclocked)", "MEMC1a (20 MHz - overclocked)", "MEMC1a (24 MHz - overclocked)"};
...
ConfigDialog::ConfigDialog(wxWindow *parent, bool is_running, int preset) {
    ...
    strncpy(config_podules[0], "arculator_rom", PODULE_SHORT_NAME_LEN - 1);
    strncpy(config_podules[1], "", PODULE_SHORT_NAME_LEN - 1);
    strncpy(config_podules[2], "", PODULE_SHORT_NAME_LEN - 1);
    strncpy(config_podules[3], "", PODULE_SHORT_NAME_LEN - 1);
    PopulatePoduleLists();
    Update5thColumn();
}
...
void ConfigDialog::UpdateList(int cpu, int mem, int memc, int fpu, int io) {
    ...
    if (romset_available_mask & (1 << config_rom))
        ...
        wxMessageBox("Configured ROM set is not available.\nYou must select an available ROM set to run this machine.", "Syracuse", wxOK | wxCENTRE | wxSTAY_ON_TOP, this);
...
void ConfigDialog::PopulatePoduleList(int slot_nr, wxComboBox *cbox) {
    ...
    if (slot_type != PODULE_NONE) {
        ...
        while (podule_get_name(c)) {
            ...
            if ((!(flags & PODULE_FLAGS_8BIT) && slot_type == PODULE_16BIT) || ((flags & PODULE_FLAGS_8BIT) && slot_type == PODULE_8BIT)) {
...
void ConfigDialog::OnOK(wxCommandEvent &event) {
    ...
    if (running) {
        if (wxMessageBox("This will reset Syracuse!\nOkay to continue?", "Syracuse", wxYES_NO | wxCENTRE | wxSTAY_ON_TOP, this) != wxYES) {
            return;
        }
    }
...
int ShowPresetList() {
    ...
    while (1) {
        ...
        wxMessageBox("You do not have any of the ROM versions required for this machine", "Syracuse", wxOK | wxCENTRE | wxSTAY_ON_TOP);
    }

    printf("Preset chosen in showpresetlist %d\n", preset);
    return preset;
}

void ShowConfigWithPreset(int preset) {
    printf("Preset chosen in showconfigwithpreset %d\n", preset);
