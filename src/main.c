...
#define SYRACUSELOG LOGDIR "syracuse.log"
...
void rpclog(const char *format, ...) {
    ...
    strncat(logfile, SYRACUSELOG, sizeof(logfile) - strlen(logfile));

    if (!rlog) {
        rlog = fopen(logfile,"wt");
...
int arc_init() {
    ...
    for (c=0;c<4;c++) {
        ...
        p = (char *)config_get_string(CFG_MACHINE, "MACHINE", s, NULL);
