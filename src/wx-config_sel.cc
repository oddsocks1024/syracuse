...
void ConfigSelDialog::OnNew(wxCommandEvent &event) {
    ...
    if (dlg.ShowModal() == wxID_OK)
    {
        ...
        if (wxFileName(config_path).Exists()) {
            wxMessageBox("A configuration with that name already exists", "Syracuse", wxOK | wxCENTRE | wxSTAY_ON_TOP, this);
        }
        else {
...
void ConfigSelDialog::OnDelete(wxCommandEvent &event)
{
        ...
        if (wxMessageBox("Are you sure you want to delete " + config_name + "?", "Syracuse", wxYES_NO | wxCENTRE | wxSTAY_ON_TOP, this) == wxYES)
        {
...
wxString GetConfigPath(wxString config_name) {
        return wxString(configdir) + config_name + wxString(".cfg");
}
