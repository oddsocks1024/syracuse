...
static sdl_render_driver_t sdl_render_drivers[] = {{RENDERER_AUTO, "auto", 1}, {RENDERER_DIRECT3D, "direct3d", 0}, {RENDERER_OPENGL, "opengl", 0}, {RENDERER_SOFTWARE, "software", 0}};
...
static int video_renderer_create(void *main_window) {
    ...
    texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, screen_rect.w, screen_rect.h);
...
int video_renderer_init(void *main_window) {
    ...
    if (main_window == NULL) {
        sdl_main_window = SDL_CreateWindow("Syracuse", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                                           768,  // 768
                                           576,  // 576
                                           0);
    } else {
    ...
    if (!sdl_main_window) {
        char message[200];
        sprintf(message, "SDL_CreateWindowFrom could not be created! Error: %s\n", SDL_GetError());
...
void video_renderer_update(BITMAP *src, int src_x, int src_y, int dest_x, int dest_y, int w, int h) {
    ...
    LOG_VIDEO_FRAMES("SDL_UpdateTexture (%d, %d)+(%d, %d) from src (%d, %d) w %d\n", texture_rect.x, texture_rect.y, texture_rect.w, texture_rect.h, src_x, src_y, src->w);
