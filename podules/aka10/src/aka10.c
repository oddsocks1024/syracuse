...
#define AKA10LOG LOGDIR "aka10.log"
...
void aka10_log(const char *format, ...) {
    ...
    strncat(logfile, AKA10LOG, sizeof(logfile) - strlen(logfile));
    va_list ap;

    if (!aka10_logf)
        aka10_logf=fopen(logfile,"wt");
...
static int aka10_init(struct podule_t *podule) {
    ...
    sprintf(rom_fn, "%s%saka10.rom", PODULEROMDIR, "aka10/");
    aka10_log("Loading AKA10 ROM %s\n", rom_fn);
