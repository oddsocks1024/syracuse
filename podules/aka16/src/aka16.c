...
#define AKA16LOG LOGDIR "aka16.log"
...
void aka16_log(const char *format, ...) {
    ...
    strncat(logfile, AKA16LOG, sizeof(logfile) - strlen(logfile));

    if (!aka16_logf)
        aka16_logf=fopen(logfile,"wt");
...
static int aka16_init(struct podule_t *podule) {
    ...
    sprintf(rom_fn, "%s%saka16.rom", PODULEROMDIR, "aka16/");
    aka16_log("Loading AKA16 ROM %s\n", rom_fn);
    f = fopen(rom_fn, "rb");

    if (!f) {
        aka16_log("Failed to load AKA16 ROM\n");
        return -1;
    }
