...
#include "arc.h"
// FIXME This should be a proper logging function
#define pclog
...
void ioctl_audio_callback(int16_t *output, int len) {
    ...
    //        ioctl_cd_end, ioctl_cd_state, cd_buflen, len, cd_buffer[4],
    //        GetTickCount());
...
static void ioctl_playaudio(uint32_t pos, uint32_t len, int ismsf) {
    ...
    if (ismsf) {
        pos = (pos & 0xff) + (((pos >> 8) & 0xff) * 75) +
              (((pos >> 16) & 0xff) * 75 * 60);
        len = (len & 0xff) + (((len >> 8) & 0xff) * 75) +
              (((len >> 16) & 0xff) * 75 * 60);
    ...
    //        ioctl_cd_end, ioctl_cd_state, 0, len);
}
...
static int read_toc(int fd, struct cdrom_tocentry *btoc) {
    ...
    for (track = 0; track < 256; track++) {
        ...
        if (err == -1) {
            //			pclog("read_toc: CDROMREADTOCENTRY failed on track
            //%i\n", track);
            continue;
        }
        c++;
        //		pclog("read_toc: %i Track %02X - number %02X control
        //%02X adr %02X address %02X %02X %02X %02X\n", c, track,
        // btoc[c].cdte_track, btoc[c].cdte_ctrl, btoc[c].cdte_adr, 0,
        // btoc[c].cdte_addr.msf.minute, btoc[c].cdte_addr.msf.second,
        // btoc[c].cdte_addr.msf.frame);
    }
...
static int ioctl_ready(void) {
    ...
    if ((toc_entry.cdte_addr.msf.minute !=
         toc[toc_hdr.cdth_trk1].cdte_addr.msf.minute) ||
        (toc_entry.cdte_addr.msf.second !=
         toc[toc_hdr.cdth_trk1].cdte_addr.msf.second) ||
        (toc_entry.cdte_addr.msf.frame !=
         toc[toc_hdr.cdth_trk1].cdte_addr.msf.frame) ||
        !tocvalid) {
        ioctl_cd_state = CD_STOPPED;
...
static int ioctl_get_last_block(unsigned char starttrack, int msf, int maxlen,
                                int single) {
    int c;
    int lb = 0;
    int tv = 0;
    ...
    for (c = 0; c <= last_track; c++) {
        uint32_t address;
        address =
            MSFtoLBA(toc[c].cdte_addr.msf.minute, toc[c].cdte_addr.msf.second,
                     toc[c].cdte_addr.msf.frame);
...
static int ioctl_readsector(uint8_t *b, int sector, int count) {
    ...
    ignore_result(read(ioctl_fd, b, count * 2048));
...
static int ioctl_readtoc(unsigned char *b, unsigned char starttrack, int msf,
    ...
    for (c = d; c <= toc_tracks; c++) {
        ...
        if ((len + 8) > maxlen)
            break;
        //                pclog("Len %i max %i Track %02X - %02X %02X
        //                %02i:%02i:%02i
        //                %08X\n",len,maxlen,toc[c].cdte_track,toc[c].cdte_adr,toc[c].cdte_ctrl,toc[c].cdte_addr.msf.minute,
        //                toc[c].cdte_addr.msf.second,
        //                toc[c].cdte_addr.msf.frame,MSFtoLBA(toc[c].cdte_addr.msf.minute,
        //                toc[c].cdte_addr.msf.second,
        //                toc[c].cdte_addr.msf.frame));
        address =
            MSFtoLBA(toc[c].cdte_addr.msf.minute, toc[c].cdte_addr.msf.second,
        ...
        if (msf) {
            ...
            temp = MSFtoLBA(toc[c].cdte_addr.msf.minute,
                            toc[c].cdte_addr.msf.second,
                            toc[c].cdte_addr.msf.frame);
            b[len++] = temp >> 24;
            b[len++] = temp >> 16;
    ...
    /*        pclog("Table of Contents (%i bytes) : \n", size);
       ...
       %02X %02X %02X %02X\n", c, toc[c].cdte_track, toc[c].cdte_ctrl,
       toc[c].cdte_adr, 0, toc[c].cdte_addr.msf.minute,
       toc[c].cdte_addr.msf.second, toc[c].cdte_addr.msf.frame); for (c = 0;c <=
       last_track; c++) pclog("Track %02X - number %02X control %02X adr %02X
       address %06X\n", c, toc[c].cdte_track, toc[c].cdte_ctrl, toc[c].cdte_adr,
       MSFtoLBA(toc[c].cdte_addr.msf.minute, toc[c].cdte_addr.msf.second,
       toc[c].cdte_addr.msf.frame));*/
    return len;
}
...
static int ioctl_readtoc_session(unsigned char *b, int msf, int maxlen) {
    ...
    if (msf) {
        ...
        uint32_t temp =
            MSFtoLBA(session.addr.msf.minute, session.addr.msf.second,
                     session.addr.msf.frame);
        b[len++] = temp >> 24;
        b[len++] = temp >> 16;
        b[len++] = temp >> 8;
        b[len++] = temp;
...
static int ioctl_readtoc_raw(unsigned char *b, int maxlen) {
    ...
    for (track = toc_hdr.cdth_trk0; track <= toc_hdr.cdth_trk1; track++) {
        if ((len + 11) > maxlen) {
            /* pclog(
                "ioctl_readtocraw: This iteration would fill the buffer beyond "
                "the bounds, aborting...\n");
            */
            return len;
        }
        ...
        //		pclog("read_toc: Track %02X - number %02X control %02X
        //adr %02X address %02X %02X %02X %02X\n", track, toc[track].cdte_track,
        // toc[track].cdte_ctrl, toc[track].cdte_adr, 0,
        // toc[track].cdte_addr.msf.minute, toc[track].cdte_addr.msf.second,
        // toc[track].cdte_addr.msf.frame);
...
void ioctl_set_drive(const char *path) {
    strncpy(cdrom_path, path, sizeof(cdrom_path) - 1);
    atapi = &ioctl_atapi;
}
...
podule_config_selection_t *cdrom_devices_config(void) {
    ...
    /*
        Search for /dev/cdrom* files. First loop counts the number of drives
        present.
    */
    dirp = opendir("/dev");
    if (dirp) {
