...
enum { ID_PATH, ID_BLOCKS, ID_SIZE, ID_ID_0, ID_ID_1, ID_ID_2, ID_ID_3, ID_ID_4, ID_ID_5, ID_ID_6, ID_TYPE_0, ID_TYPE_1, ID_TYPE_2, ID_TYPE_3, ID_TYPE_4, ID_TYPE_5, ID_TYPE_6 };
...
int change_path(void *window_p, const struct podule_config_item_t *item, void *new_data) {
    ...
    if (!podule_callbacks->config_file_selector(window_p, "Please enter a file name", NULL, NULL, NULL, "HDF files (*.hdf)|*.hdf", fn, sizeof(fn), CONFIG_FILESEL_SAVE)) {
        //                rpclog("Filename: %s\n", fn);
...
static podule_config_t scsi_new_drive_config = {.title = "New drive",
                                                .init = new_drive_init,
                                                .close = new_drive_close,
                                                .items = {{.description = "Path :", .type = CONFIG_STRING, .flags = CONFIG_FLAGS_DISABLED, .default_string = "", .id = ID_PATH},
                                                          {.description = "Change path...", .type = CONFIG_BUTTON, .function = change_path},
                                                          {.description = "Blocks :", .type = CONFIG_STRING, .default_string = "102400", .id = ID_BLOCKS, .function = changed_blocks},
                                                          {.description = "Size (MB) :", .type = CONFIG_STRING, .default_string = "50", .id = ID_SIZE, .function = changed_size},
                                                          {.type = -1}}};
...
/* Not currently used */
/*
static podule_config_t scsi_load_drive_config = {.title = "Drive geometry",
                                                 .init = drive_load_open,
                                                 .close = drive_load_close,
                                                .items = {{.description = "Blocks :", .type = CONFIG_STRING, .default_string = "102400", .id = ID_BLOCKS, .function = changed_blocks},
                                                           {.description = "Size (MB) :", .type = CONFIG_STRING, .default_string = "50", .id = ID_SIZE, .function = changed_size},
                                                           {.type = -1}}};
*/
...
static int config_load_drive(void *window_p, const struct podule_config_item_t *item, void *new_data) {
    ...
    if (!podule_callbacks->config_file_selector(window_p, "Please select a hard drive image", NULL, NULL, NULL, "HDF files (*.hdf)|*.hdf", fn, sizeof(fn), CONFIG_FILESEL_LOAD)) {
        FILE *f;
        int filesize;
        char temp_s[256];
...
static podule_config_t scsi_drive_edit_config = {
    ...
    .items = {{.name = "fn", .description = "Path :", .type = CONFIG_STRING, .flags = CONFIG_FLAGS_DISABLED | CONFIG_FLAGS_NAME_PREFIXED, .id = ID_PATH},
              {.name = "size", .description = "Blocks :", .type = CONFIG_STRING, .flags = CONFIG_FLAGS_DISABLED | CONFIG_FLAGS_NAME_PREFIXED, .id = ID_BLOCKS},
              {.description = "Size (MB) :", .type = CONFIG_STRING, .flags = CONFIG_FLAGS_DISABLED, .default_string = "50", .id = ID_SIZE, .function = changed_size},
              {.description = "New drive...", .type = CONFIG_BUTTON, .function = config_new_drive},
              {.description = "Load drive...", .type = CONFIG_BUTTON, .function = config_load_drive},
              {.description = "Eject drive", .type = CONFIG_BUTTON, .function = config_eject_drive},
              {.type = -1}}};

static podule_config_t scsi_cdrom_config = {
    .title = "CD-ROM drive configuration",
    .items = {{.name = "fn", .description = "Host CD-ROM drive", .type = CONFIG_SELECTION_STRING, .flags = CONFIG_FLAGS_NAME_PREFIXED, .selection = NULL, .default_string = ""}, {.type = -1}}};

static podule_config_t scsi_invalid_config = {.title = "No drive to configure", .items = {{.type = -1}}};
...
static podule_config_selection_t device_type_sel[] = {{.description = "None", .value_string = "none"},
                                                      {.description = "Hard drive", .value_string = "hd"},
                                                      {.description = "CD-ROM drive", .value_string = "cd"},
                                                      {.description = "", .value_string = ""}};

podule_config_t scsi_podule_config = {
    .title = "SCSI device configuration",
    .init = scsi_podule_config_init,
    .items = {{.name = "device0_type", .description = "ID 0 type", .type = CONFIG_SELECTION_STRING, .selection = device_type_sel, .default_string = "none", .function = change_type, .id = ID_TYPE_0},
              {.description = "Configure ID 0...", .type = CONFIG_BUTTON, .function = config_drive, .id = ID_ID_0},
              {.name = "device1_type", .description = "ID 1 type", .type = CONFIG_SELECTION_STRING, .selection = device_type_sel, .default_string = "none", .function = change_type, .id = ID_TYPE_1},
              {.description = "Configure ID 1...", .type = CONFIG_BUTTON, .function = config_drive, .id = ID_ID_1},
              {.name = "device2_type", .description = "ID 2 type", .type = CONFIG_SELECTION_STRING, .selection = device_type_sel, .default_string = "none", .function = change_type, .id = ID_TYPE_2},
              {.description = "Configure ID 2...", .type = CONFIG_BUTTON, .function = config_drive, .id = ID_ID_2},
              {.name = "device3_type", .description = "ID 3 type", .type = CONFIG_SELECTION_STRING, .selection = device_type_sel, .default_string = "none", .function = change_type, .id = ID_TYPE_3},
              {.description = "Configure ID 3...", .type = CONFIG_BUTTON, .function = config_drive, .id = ID_ID_3},
              {.name = "device4_type", .description = "ID 4 type", .type = CONFIG_SELECTION_STRING, .selection = device_type_sel, .default_string = "none", .function = change_type, .id = ID_TYPE_4},
              {.description = "Configure ID 4...", .type = CONFIG_BUTTON, .function = config_drive, .id = ID_ID_4},
              {.name = "device5_type", .description = "ID 5 type", .type = CONFIG_SELECTION_STRING, .selection = device_type_sel, .default_string = "none", .function = change_type, .id = ID_TYPE_5},
              {.description = "Configure ID 5...", .type = CONFIG_BUTTON, .function = config_drive, .id = ID_ID_5},
              {.name = "device6_type", .description = "ID 6 type", .type = CONFIG_SELECTION_STRING, .selection = device_type_sel, .default_string = "none", .function = change_type, .id = ID_TYPE_6},
              {.description = "Configure ID 6...", .type = CONFIG_BUTTON, .function = config_drive, .id = ID_ID_6},
              {.type = -1}}};
